...
#include <dharma/dynamic_tree_vote.h>
#include <dharma/allreduce.h>
#include <dharma/allgather.h>
#include <dharma/domain.h>
#include <dharma/bcast.h>
#include <sprockit/stl_string.h>
#include <sprockit/sim_parameters.h>
#include <sprockit/keyword_registration.h>

RegisterDebugSlot(dharma);
ImplementFactory(dharma::transport);
...
transport::validate_api()
{
  if (!inited_ || finalized_){
    ...
    "DHARMA transport calling function while not inited or already finalized");
  }
}
...
transport::init()
{
  ...
  const char* nspare_str = getenv("DHARMA_NUM_SPARES");
  if (nspare_str){
...
transport::handle(const message::ptr& msg)
{
  ...
  case message::collective: {
    ...
    if (it == collectives_[ty].end()){
      debug_printf(sprockit::dbg::dharma_collective_sendrecv,
...
transport::start_watching(int dst, timeout_function *func)
{
  ...
  if (is_failed(dst)){
    ...
    debug_printf(sprockit::dbg::dharma | sprockit::dbg::dharma_ping,
...
transport::start_collective(collective* coll)
{
  ...
  if (tag >= heartbeat_tag_start_ && tag <= heartbeat_tag_stop_){
    spkt_throw_printf(sprockit::value_error,
        "dharma::start_collective: %s tag %d is reserved for heartbeats",
        collective::tostr(ty), tag);
  }
...
transport::smsg_send(int dst, message::payload_type_t ev, const message::ptr& msg, bool needs_ack)
{
  ...
  debug_printf(sprockit::dbg::dharma,
    "Rank %d DHARMA sending short message to %d",
    rank_, dst);
...
{
  bool needs_send_ack = msg->class_type() != dharma::message::ping;
