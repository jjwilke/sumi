...
#ifndef dharma_THREAD_SAFE_PTR_TYPE_H
#define dharma_THREAD_SAFE_PTR_TYPE_H
...
#include <dharma/thread_safe_refcount_ptr.h>
...
dharma::thread_safe_refcount_ptr<Out>
__safe_ptr_cast__(const char* objname,
              const char* file,
              int line,
              const dharma::thread_safe_refcount_ptr<In>& in,
              const char* error_msg = "error")
{
  dharma::thread_safe_refcount_ptr<Out> out = dynamic_cast<Out*>(in.get());
  if (!out) {
...
*/
#define dharma_ptr_safe_cast(type,...) \
    ::dharma::__safe_ptr_cast__<type>(#type, __FILE__, __LINE__, __VA_ARGS__)

} // end of namespace dharma
